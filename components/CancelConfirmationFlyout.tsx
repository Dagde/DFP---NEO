...
interface CancelConfirmationFlyoutProps {
  onConfirm: () => void;
  onClose: () => void;
}

const CancelConfirmationFlyout: React.FC<CancelConfirmationFlyoutProps> = ({ onConfirm, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black/70 z-[80] flex items-center justify-center animate-fade-in" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md border border-amber-500/50" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-gray-700 bg-amber-900/20 flex items-center space-x-3">
                    ...
                    <h2 className="text-xl font-bold text-amber-400">Confirm Cancellation</h2>
                </div>
                <div className="p-6">
                    <p className="text-gray-300">
                        Are you sure you wish to permanently remove this event from the DFP?
                    </p>
...
export default CancelConfirmationFlyout;