...
const AddUnavailabilityFlyout: React.FC<AddUnavailabilityFlyoutProps> = ({ onClose, onTodayOnly, onSave, unavailabilityPeriods, onRemove }) => {
    ...
    const handleSave = () => {
        if (!startDate || !endDate) {
            alert('Please provide a full start and end date.');
            return;
        }

        // The logic for 'end date' is that it's the first day of return.
        // For an all-day event, if start and end are the same, it means 0 days unavailable.
        // So the end date must be at least one day after the start date.
        const isAllDay = !startTime && !endTime;
        if (isAllDay && new Date(endDate) <= new Date(startDate)) {
             alert('For an all-day unavailability, the "End Unavailable" date must be at least one day after the start date.');
             return;
        }
        
        // For a timed event on a single day, the end date can be the same as the start date.
        if (!isAllDay && new Date(endDate) < new Date(startDate)) {
