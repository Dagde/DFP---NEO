                ...
                <div className="p-4 overflow-y-auto flex-1 space-y-4">
                    {phraseData ? (
                        ...
                        <div className="text-center py-8 text-gray-500 italic">
                            ...
                            <p className="text-xs mt-2">Configure in Settings &rarr; Scoring Matrix.</p>
                        </div>
                    )}
                </div>
...
const PT051View: React.FC<PT051ViewProps> = ({ trainee, event, onBack, onSave, initialAssessment, instructors, pt051Assessments, events, lmpScores, syllabusDetails, registerDirtyCheck, phraseBank }) => {
        ...
        <div className="flex-1 flex flex-col bg-gray-900 overflow-y-auto">
            ...
            <div className="flex-shrink-0 bg-gray-800 p-4 flex justify-between items-center border-b border-gray-700 sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-white">PT-051 Assessment</h1>
                        <p className="text-sm text-gray-400">Flight: {event.flightNumber} on {event.date} at {formatTime(event.startTime)}</p>
                    </div>
            ...
            <div className="p-4 md:p-6 w-full max-w-full mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    <dl className="lg:col-span-1 space-y-2 p-4 bg-gray-800 border border-gray-700 rounded-lg h-fit">
                        <InfoField label="Trainee" value={`${trainee.rank} ${trainee.name}`} />
                        <InfoField label="Course" value={trainee.course} />
                        <InfoField label="Date" value={event.date} />
                        <InfoField label="Time" value={`${formatTime(event.startTime)} - ${formatTime(event.startTime + event.duration)}`} />
                         <div className="col-span-2">
                             <InfoField label="QFI" value={qfi ? `${qfi.rank} ${qfi.name}` : 'N/A'} />
                         </div>
                    </dl>
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        <fieldset className="p-4 border border-gray-600 rounded-lg">
                            ...
                            {/* DCO/DPCO/DNCO Radio Buttons - For Flight, FTD, and Ground events */}
                            {(event.type === 'flight' || event.type === 'ftd' || event.type === 'ground') && (
                             ...
                             <div className="mt-2 space-y-4">
                                    ...
                                    <div className="mt-1 flex justify-around p-2 bg-gray-700/50 rounded">
                                        {OVERALL_GRADES.map(grade => (
                                            <label key={grade} className="flex flex-col items-center space-y-1 text-xs text-gray-300 cursor-pointer">
                                                <span>{grade}</span>
                                                <input type="radio" name="overall-grade" value={grade} checked={overallGrade === grade} onChange={() => setOverallGrade(grade)} disabled={event.type === 'ground'} className={`h-4 w-4 ${getOverallRadioAccentColor(grade)} bg-gray-600 ${event.type === 'ground' ? 'opacity-50 cursor-not-allowed' : ''}`} />
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="mt-1 flex space-x-4">
                                        <label className={`cursor-default rounded-lg p-4 w-1/2 text-center transition-all duration-200 ${
                                            overallResult === 'P'
                                                ? 'bg-green-600 text-white ring-2 ring-white scale-105 shadow-lg'
                                                : 'bg-green-800/50 text-green-200'
                                            } ${overallResult === null ? '!bg-gray-700 !text-gray-500' : ''}`}
                                        >
                                            <input type="radio" name="overall-result" value="P" checked={overallResult === 'P'} readOnly className="sr-only" />
                                        ...
                                        <label className={`cursor-default rounded-lg p-4 w-1/2 text-center transition-all duration-200 ${
                                            ...
                                            } ${overallResult === null && !showDoubleMarginalWarning ? '!bg-gray-700 !text-gray-500' : ''}`}
                                        >
                                            <input type="radio" name="overall-result" value="F" checked={overallResult === 'F'} readOnly className="sr-only" />
