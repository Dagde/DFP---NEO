            ...
            <tbody className="divide-y divide-gray-700/50">
                {events.map(event => (
                    <tr key={event.id} onClick={() => onSelectEvent(event)} className="hover:bg-sky-900/50 transition-colors cursor-pointer">
                        <td className="py-2 px-2 text-gray-300">{event.student || 'N/A'}</td>
                        <td className="py-2 px-2 text-gray-300 font-semibold">{event.flightNumber}</td>
                        <td className="py-2 px-2 text-gray-300">{event.soloOrDual || 'N/A'}</td>
                        <td className="py-2 px-2 text-gray-300">{event.currency || 'N/A'}</td>
                        <td className="py-2 px-2 text-gray-300">{event.currencyExpire || 'N/A'}</td>
                        <td className="py-2 px-2 text-gray-300 font-mono">{new Date().toLocaleDateString('fr-FR')}</td>
