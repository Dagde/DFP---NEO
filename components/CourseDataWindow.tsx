...
const CourseDataWindow: React.FC<CourseDataWindowProps> = ({
    ...
    const traineePoints = useMemo(() => {
        return courseData.trainees.map(t => ({
            name: t.trainee.name,
            eventsCompleted: t.completedCount,
        }));
    }, [courseData.trainees]);
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 flex flex-col h-fit">
            ...
            <div className="p-4 border-t border-gray-700">
                <button
                    ...
                    className="w-full text-center px-4 py-2 bg-gray-700 text-gray-300 rounded-md hover:bg-gray-600 hover:text-white transition-colors text-sm font-semibold"
                >
            ...
            {showGraph && (
                <div className="p-4 border-t border-gray-700">
                    <CourseProgressGraph
                        startDate={startDate}
                        gradDate={gradDate}
                        totalEvents={courseData.totalSyllabusEvents}
                        traineePoints={traineePoints}
