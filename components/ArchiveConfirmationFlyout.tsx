...
const ArchiveConfirmationFlyout: React.FC<ArchiveConfirmationFlyoutProps> = ({ instructorName, onConfirm, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black/70 z-[70] flex items-center justify-center animate-fade-in" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md border border-amber-500/50" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-gray-700 bg-amber-900/20 flex items-center space-x-3">
                    ...
                    <h2 className="text-xl font-bold text-amber-400">Confirm Archive</h2>
                </div>
                <div className="p-6">
                    <p className="text-gray-300">
                        Are you sure you want to archive <strong className="text-white">{instructorName}</strong>?
                    </p>
                    <p className="text-gray-400 mt-2 text-sm">
                        This will move them to the archive bin. You can restore them later.
                    </p>
                </div>
...
export default ArchiveConfirmationFlyout;
